...
In this example, the E-W Firewall is deployed in transparent mode, acting as the default gateway for the server C subnet. Please note that IP rules require that a host and its default gateway be in the same subnet, i.e., be able to exchange layer two frames.
