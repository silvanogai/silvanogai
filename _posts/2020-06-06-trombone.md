...
Not every  trombone is a bottleneck. Historically tromboning was happening towards the WAN, a typical example being the firewall connecting a corporate network to the Internet. Usually , the capacity of the link to the service provider was the bottleneck.
...
When applied to  East-West traffic, the firewall trombone may become a bottleneck. In the same post, we have also discussed how placing the trombone on a Clos network disrupts optimal routing and causes a large percentage of the network bandwidth to go wasted.
